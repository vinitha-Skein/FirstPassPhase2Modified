...
extension ReportsViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tableview
        {
            ...
            cell.downloadButton.addTarget(self, action: #selector(downloadButtonPessed(sender:)), for: .touchUpInside)
            return cell
        }
        else
        {
    ...
    @objc func downloadButtonPessed(sender: UIButton){
        downloadReport()
    }

    func downloadReport(){
        do {
        let stuffFileURL = try makeWritableCopy(named: "Report.pdf", ofResourceFile: "Report.pdf")
